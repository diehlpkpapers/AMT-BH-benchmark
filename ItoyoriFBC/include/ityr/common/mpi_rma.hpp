...
// ---- RMA op counters (profiling only; harmless when unread) ----
inline thread_local std::size_t tl_rma_get = 0;
inline thread_local std::size_t tl_rma_get_bytes = 0;
inline thread_local std::size_t tl_rma_put = 0;
inline thread_local std::size_t tl_rma_put_bytes = 0;
inline thread_local std::size_t tl_rma_sync = 0;
inline thread_local std::size_t tl_rma_flush = 0;
inline thread_local std::size_t tl_rma_faa = 0;
...
inline void mpi_win_sync(MPI_Win win) {
  MPI_Win_sync(win);
  tl_rma_sync += 1;
}

// Write back dirty cache lines covering [p, p+bytes) so that RMA operations
// (MPI_Get/DMA reads of the registered window memory) observe the data.
// Plain stores can sit in the CPU write-back cache and be invisible to
// remote GETs; MPI_Win_sync is only a fence and does not force write-back.
//
// On I/O-coherent platforms (RDMA NICs snooping the CPU caches) the explicit
// write-back is unnecessary: the NIC DMA reads observe CPU stores through
// cache coherence, and MPI_Win_sync provides the required ordering. Default
// ON; disable with ITYR_CACHE_FLUSH=0 on coherent hardware after validating
// correctness.
inline bool cache_flush_enabled() {
  static bool on = [] {
    const char* s = std::getenv("ITYR_CACHE_FLUSH");
    return !(s && (s[0] == '0' || s[0] == 'f' || s[0] == 'F'));
  }();
  return on;
}

inline void cache_flush(void* p, std::size_t bytes) {
  ...
  char* end = q + bytes;
  // clflush works on unaligned addresses; flush each 64-byte line once.
  const std::size_t line = 64;
  char* start = reinterpret_cast<char*>(reinterpret_cast<std::uintptr_t>(q) & ~(line - 1));
  for (char* c = start; c < end; c += line) _mm_clflush(c);
  asm volatile("" ::: "memory");
#else
  (void)p; (void)bytes;
#endif
...
inline void mpi_get(T*          origin,
  ...
  tl_rma_get += 1;
  tl_rma_get_bytes += sizeof(T) * count;
  mpi_win_flush(target_rank, win);
}
...
public:
  ...
  std::span<T> local_buf() const { return local_buf_; }

private:
  template <typename... ElemArgs>
  std::span<T> init_local_buf(std::size_t count, ElemArgs... args) const {
    ...
    mpi_barrier(comm_);
    return std::span<T>(local_base, count);
  }
