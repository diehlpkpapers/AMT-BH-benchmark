...
public:
  ...
  void* do_allocate(std::size_t bytes, std::size_t alignment) override {
    ...
    if (!s.has_value()) {
      // Dump the freelist state to distinguish exhaustion (total ~ 0) vs
      // fragmentation (total large but max < request) vs corruption
      // (total+in-use inconsistent with window).
      auto st = freelist_.stats();
      fprintf(stderr,
              "[ityr::common::allocator] malloc_local() FAILED: req=%zu align=%zu "
              "freelist={count=%zu total=%zu max=%zu}\n",
              real_bytes, alignment, st.count, st.total, st.max);
      fflush(stderr);
  ...
  freelist::stat stats() const { return freelist_.stats(); }
...
public:
  remotable_resource(std::size_t local_max_size)
    : local_max_size_(calc_local_max_size(local_max_size)),
      ...
      vm_(rma_use_mpi_win_allocate::value()
              ? virtual_mem()
              : reserve_same_vm_coll(global_max_size_, local_max_size_)),
      pm_(rma_use_mpi_win_allocate::value() ? physical_mem() : init_pm()),
      win_(init_win_mr(vm_, local_max_size_)),
      local_base_addr_(reinterpret_cast<std::byte*>(win_.baseptr())),
      win_mr_(local_base_addr_, local_max_size_, win_.win()),
  ...
  // With ITYR_RMA_USE_MPI_WIN_ALLOCATE=1 the pool memory is MPI-allocated
  // (per-rank bases) instead of a same-address vm reservation; derive the
  // global (rank-0) base from our own window base so that get_owner/get_disp
  // keep working (enables osc=sm for the data path).
  std::byte* global_base_addr() const {
    if (rma_use_mpi_win_allocate::value()) {
      return reinterpret_cast<std::byte*>(win_.baseptr()) -
             local_max_size_ * topology::my_rank();
    }
    return reinterpret_cast<std::byte*>(vm_.addr());
  }

  static mpi_win_manager<std::byte> init_win_mr(const virtual_mem& vm, std::size_t local_max_size) {
    if (rma_use_mpi_win_allocate::value()) {
      return {topology::mpicomm(), local_max_size};
    } else {
      auto local_base = reinterpret_cast<std::byte*>(vm.addr()) + local_max_size * topology::my_rank();
      return {topology::mpicomm(), local_base, local_max_size};
    }
  }

  bool has(const void* p) const {
    auto base = global_base_addr();
    return base <= p && p < base + global_max_size_;
  }

  topology::rank_t get_owner(const void* p) const {
    return (reinterpret_cast<uintptr_t>(p) - reinterpret_cast<uintptr_t>(global_base_addr())) / local_max_size_;
  }
  ...
  void* do_allocate(std::size_t bytes, std::size_t alignment = alignof(max_align_t)) override {
    ...
    // Trace big (>=1 MiB) pool allocations to show the growth curve of the
    // large buffers (get() copies, concat buffers). Gated by ITYR_DEBUG_POOL=1.
    peak_allocated_size_ = std::max(peak_allocated_size_, allocated_size_);
    if (real_bytes >= (1u << 20) && std::getenv("ITYR_DEBUG_POOL")) {
      auto fs = win_mr_.stats();
      fprintf(stderr,
              "[ityrbig] rank=%d alloc=%.1fMiB live=%.1fMiB peak=%.1fMiB "
              "free={count=%zu total=%.1fMiB max=%.1fMiB}\n",
              topology::my_rank(), real_bytes / 1048576.0, allocated_size_ / 1048576.0,
              peak_allocated_size_ / 1048576.0, fs.count, fs.total / 1048576.0,
              fs.max / 1048576.0);
      fflush(stderr);
    }
...
//    ityr::common::verbose("get_owner(%p) = %d\n", p, get_owner(p));
...
private:
  static std::string allocator_shmem_name(int inter_rank) {
    static int count = 0;
    std::stringstream ss;
    ss << "/ityr_allocator_" << count++ << "_" << inter_rank << "_" << getuid();
    return ss.str();
  }
  ...
  mpi_win_manager<std::byte> create_win() const {
    if constexpr (use_dynamic_win) {
      ...
      //printf("rank=%d create_win() with { %u, %p, %u }", topology::my_rank(), topology::mpicomm(), local_base_addr, local_max_size_);
  ...
  std::size_t                       peak_allocated_size_ = 0;
...
void remote_get(const remotable_resource& rmr, T* origin_p, const T* target_p, std::size_t size) {
  if (rmr.is_locally_accessible(target_p)) {
    ityr::common::verbose("remote_get size=%u, sizeof(T)=%u", size * sizeof(T), sizeof(T));
...
template <typename T>
T remote_cas_value(const remotable_resource& rmr, const T& val, const T& compare, T* target_p) {
  auto target_rank = rmr.get_owner(target_p);
  return common::mpi_atomic_cas_value(val, compare, target_rank, rmr.get_disp(target_p), rmr.win());
}
