...
public:
  ...
  void lock(topology::rank_t target_rank, int idx = 0) const {
    ...
    while (!trylock(target_rank, idx)) {
      // Make MPI progress while spinning so that remote RMA operations (e.g.
      // a thief's frame GET) can complete; without this, a progress-starved
      // peer can deadlock the lock.
  ...
  void priolock(topology::rank_t target_rank, int idx = 0) const {
    ...
    while (mpi_atomic_get_value<lock_t>(target_rank, get_disp(idx), lock_win_.win()) != 1) {
      // Make MPI progress while spinning (see lock()).
      common::mpi_make_progress();
    }
  }
